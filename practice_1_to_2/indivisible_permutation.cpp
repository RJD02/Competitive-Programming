...
template <typename T>
T logAToBaseB(T a, T b) {
  return a > b - 1 ? 1 + logAToBaseB(a / b, b) : 0;
}
template <typename T>
T logAToBaseBDec(T a, T b) {
  return log(a) / log(b);
}
template <typename T>
T numberOfDigits(T a) {
  return (int)floor(1 + logAToBaseBDec(a, 10));
}
template <typename T>
double nthRoot(T a, T b) {
  double xPre = rand() % 10;
  double eps = 1e-3;
  double delX = INT_MAX;
  double xK;
...
int main() {
  ...
  // Remember, you are loved and your obsession is to be better, not perfect. Go
  // get it.
  while (T--) solve();
