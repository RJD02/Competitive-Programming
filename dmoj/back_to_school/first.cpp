...
void solve() {
	...
	for(auto i: v) 
		ans.push_back(i);
	for(int i = 0; i < n; i++) {
		bool isABefore = false;
		bool isBAfter = false;
		for(int j = 0; j < m; j++) {
			// if(v[i][j] == "A" and isBBefore) cout << "N";
			if(v[i][j] == 'B' and isABefore and pos <= k) ans[i][j] = 'N';
			else if(v[i][j] == 'B' and !isABefore) ans[i][j] = 'Y';
			else if (v[i][j] == 'B' and isABefore and pos > k) ans[i][j] == 'Y';
			else if(v[i][j] == 'A') isABefore = true, pos = j;
			
		}
		for(int j = m - 1; j >= 0; j--) {
			if(v[i][j] == 'A' and isBAfter) ans[i][j] = 'N';
			else if(v[i][j] == 'B') isBAfter = true;
			else if(v[i][j] == 'A' and !isBAfter) ans[i][j] = 'Y';
			// bug(isBAfter);
		}
		// cout << endl;
	}
