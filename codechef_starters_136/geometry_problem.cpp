...
int bfs(string s) {
    ...
    if(s[0] == '1') dir.push_back({-1, 0});
    else dir.push_back({0,0});

    if(s[1] == '1') dir.push_back({1, 0});
    else dir.push_back({0, 0});

    if(s[2] == '1') dir.push_back({0, 1});
    else dir.push_back({0,0});

    if(s[3] == '1') dir.push_back({0, -1});
    else dir.push_back({0,0});
    ...
    q.push({0,0});
    visited[10][10] = true; // Adjust initial position to fit array indices
    ...
    while(!q.empty()) {
        ...
        for(auto j: dir) {
            int x = j.first;
            int y = j.second;
            int dest_x = node.first + x;
            int dest_y = node.second + y;
            
            if(dest_x <= 10 and dest_x >= -10 and dest_y <= 10 and dest_y >= -10 and !visited[dest_x+10][dest_y+10]) {
                q.push({dest_x, dest_y});
                visited[dest_x+10][dest_y+10] = true;
            }
        }
        count++; // Increment count for each valid move from the current position
    }
